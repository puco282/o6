...
def generate_image(prompt):
    ...
    return Image.open(io.BytesBytesIO(image_data))
...
if chat_option.startswith("1"):
    ...
    # st.session_state 초기화 (재접속 시에도 올바르게 작동하도록)
...
학생의 이야기를 **먼저 면밀히 읽고 분석해.** 그리고 나서 이야기에 **이해되지 않거나 더 구체화가 필요한 부분이 있다면** **반드시 한 번에 하나의 질문만**을 통해 학생 스스로 생각하고 답하도록 유도해.
...
1.  **이야기 요소 구체화 (전체적인 것부터 세부적인 것으로):**
    - **만약 이야기에 주인공에 대한 설명이 부족하거나 모호하다면:** '주인공은 어떤 아이인가요? 이름이나 특별한 점이 있나요?'
    - **만약 이야기에 배경 설명이 부족하거나 분위기가 불분명하다면:** '이야기의 배경은 어떤 곳인가요? 어떤 분위기인가요?'
    - **만약 이야기에 가장 먼저 일어나는 중요한 사건이 불분명하다면:** '이야기에서 가장 먼저 일어나는 중요한 사건은 무엇인가요?'
    - **만약 이야기에 주인공 외의 인물 설명이 부족하거나 관계가 불분명하다면:** '주인공 외에 또 어떤 인물이 등장하나요? 그 인물은 주인공과 어떤 관계인가요?'
2.  **사건의 인과관계 및 흐름 확인:**
    - **만약 사건의 발생 원인이나 연결이 불분명하다면:** '이 사건이 왜 일어났나요? 무엇 때문에 이런 일이 벌어졌나요?'
    - **만약 사건들 간의 연결이 어색하거나 논리적이지 않다면:** '앞선 사건과 지금 사건은 어떻게 연결되나요? 사건들이 논리적으로 이어지나요?'
    - **만약 이야기의 전반적인 흐름이 자연스럽지 않다면:** '이야기의 흐름이 자연스럽게 느껴지나요? 혹시 갑자기 이야기가 바뀌는 부분은 없나요?'
3.  **인물의 감정 및 생각:**
    - **만약 특정 상황에서 인물의 감정이나 생각이 충분히 드러나지 않는다면:** '이 상황에서 주인공은 어떤 기분이었을까요? 어떤 생각을 했을까요?'
    - **만약 인물의 감정이나 생각이 이야기 전개에 미치는 영향이 불분명하다면:** '주인공의 감정이나 생각이 이야기 전개에 어떤 영향을 주었나요?'
4.  **이야기 구조(발단-전개-절정-결말) 점검 (질문을 통해 유도):**
    - **만약 이야기의 시작 부분이 불분명하다면:** '이야기가 어떻게 시작되었나요? 발단 부분에서 무엇을 보여주고 싶었나요?'
    - **만약 주요 사건들의 전개나 갈등 발전이 불분명하다면:** '주요 사건들이 어떻게 전개되나요? 갈등은 어떻게 발전하나요?'
    - **만약 이야기의 절정 부분이 불분명하거나 약하다면:** '이 이야기에서 가장 중요한 순간, 즉 절정은 언제인가요? 왜 그 순간이 가장 흥미롭거나 결정적인가요? 주인공에게 어떤 큰 변화가 있었나요?'
    - **만약 이야기의 마무리나 결말 부분이 불분명하다면:** '이야기는 어떻게 마무리되나요? 결말 부분에서 주인공에게 어떤 변화가 있었나요?'
5.  **어색하거나 불분명한 문장/표현 (질문을 통해 유도):**
    - **만약 특정 문장이나 표현이 어색하거나 문맥에 맞지 않는다고 판단되면:** 해당 문장을 직접 고쳐주기보다는 간단한 예시를 들어 학생이 스스로 수정할 수 있도록 조언해줘. 이 조언 또한 단일 질문과 함께 제공될 수 있어.
...
항상 학생의 창의성을 존중하고 칭찬과 격려의 말투를 꼭 유지해줘."""
        ...
        # st.session_state에 story_input_submitted가 없으면 False로 초기화
        if "story_input_submitted" not in st.session_state:
            st.session_state.story_input_submitted = False 

    # --- 기존 코드 수정: story_input_submitted가 True일 때만 메시지 표시/입력 창 활성화 ---
