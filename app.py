...
질문은 다음 영역들을 참고하여 유연하게 진행할 수 있어:
-   **이야기 요소 구체화:** 주인공(이름, 성격, 특징), 배경(장소, 시간, 분위기), 주요 사건(시작, 전개, 결말), 등장인물(관계, 역할) 등 이야기의 핵심 요소 중 불분명하거나 더 깊이 탐구할 부분이 있다면 질문해.
        ...
        if st.session_state.messages_image_generation and \
           ...
           not st.session_state.image_prompt_collected: # Only collect once
            ...
            if end_index == -1:
                ...
                final_dalle_prompt = gpt_last_prompt_content[start_index:end_index].strip() # 오타 수정
