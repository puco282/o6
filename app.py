    ...
    if "messages_story_review" not in st.session_state:
        st.session_state.messages_story_review = [
            {"role": "system", "content": (
                ...
                "- **이야기 요소 구체화:** '주인공은 어떤 아이인가요? 이야기의 배경은 어떤 곳인가요? 어떤 주요 사건들이 발생하나요? 주인공 외에 어떤 인물이 더 등장하나요?'\n"
                "- **사건의 인과관계 및 흐름:** '이 사건이 왜 일어났나요? 앞선 사건과 어떻게 연결되나요? 사건들이 논리적으로 이어지나요? 이야기의 흐름이 자연스럽게 느껴지나요?'\n"
                "- **인물의 감정 및 생각:** '이 상황에서 주인공은 어떤 기분이었을까요? 어떤 생각을 했을까요?' (이 질문은 이야기의 명확성과 흐름을 돕는 맥락에서 사용)\n"
                "- **이야기 구조 점검 (발단-전개-절정-결말):**\n"
                ...
                "- **어색하거나 문맥에 맞지 않는 표현:**\n"
                "    - 만약 특정 문장이나 표현이 어색하거나 문맥에 맞지 않는다고 판단되면, **해당 문장을 인용하며** 학생이 스스로 더 명확하게 표현하도록 유도하는 질문과 **간단한 예시**를 제공해.\n"
                "    - 예시 질문: '이 문장 (\'...\')이 어떤 의미인지 조금 더 자세히 설명해 줄 수 있을까요? 예를 들어, \'...\'처럼 표현하면 더 명확하게 전달될 수도 있을 것 같아요.'\n"
                "    - 예시 질문: '이 표현(\'...\')이 이야기 속에서 어떤 느낌을 주나요? 혹시 다른 단어나 문장으로 바꾸면 더 생생하게 전달될까요?'\n"
                "\n"
                "학생의 답변을 바탕으로 다음 질문을 이어나가고, 이야기가 충분히 구체화되었다고 판단되면 **최종적으로 다음과 같은 평가와 수정 방안을 구체적이고 세세하게, 그리고 쉽게 설명해줘.**\n"
                "1. **이야기 구조 평가:** '이야기는 발단-전개-절정-결말의 구조를 잘 따르고 있어요.' 또는 '이야기 구조에서 발단 부분이 조금 더 명확해지면 좋을 것 같아요. 예를 들어...'와 같이 명확하게 평가해줘.\n"
                "2. **흐름 및 문맥 평가:** '이야기의 흐름이 전반적으로 자연스럽고 사건들이 잘 연결되어 있어요.' 또는 '이 부분에서 사건의 연결이 조금 어색하게 느껴져요. 예를 들어...'와 같이 명확하게 평가해줘.\n"
                "3. **문장 표현 평가:** '문장들이 대부분 명확하고 생생하게 표현되어 있어요.' 또는 '몇몇 문장(예: \'...\')이 조금 어색하게 느껴져요. 예를 들어, 이 부분을 \'...\'처럼 바꾸면 더 좋을 것 같아요.'와 같이 명확하게 평가하고 수정 예시를 제시해줘.\n"
                "4. **종합적인 수정 방안 및 Pika 영상 제작 조언:** 위 평가를 종합하여 이야기의 보완 방향을 구체적이고 세세하며 학생들이 이해하기 쉽게 설명해줘. Pika 영상 제작을 위해 어떤 부분을 더 강조하거나 간결하게 표현하면 좋을지 등 실질적인 조언을 포함해줘.\n"
                "\n"
    ...
    if st.session_state.story_input_submitted and st.button("대화 초기화", key="reset_story_review_chat"):
        st.session_state.messages_story_review = [
            {"role": "system", "content": (
                ...
                "    - '이야기는 어떻게 마무리되나요? 결말 부분에서 주인공에게 어떤 변화가 있었나요?'\n"
                "- **어색하거나 불분명한 문장:**\n"
                "    - 만약 특정 문장이나 표현이 어색하거나 불분명하다고 판단되면, 해당 문장을 직접 고쳐주지 말고, **해당 문장을 인용하며** 학생이 스스로 더 명확하게 표현하도록 유도하는 질문과 **간단한 예시**를 제공해.\n"
                "    - 예시 질문: '이 문장 (\'...\')이 어떤 의미인지 조금 더 자세히 설명해 줄 수 있을까요? 예를 들어, \'...\'처럼 표현하면 더 명확하게 전달될 수도 있을 것 같아요.'\n"
                "    - 예시 질문: '이 표현(\'...\')이 이야기 속에서 어떤 느낌을 주나요? 혹시 다른 단어나 문장으로 바꾸면 더 생생하게 전달될까요?'\n"
                "\n"
                "학생의 답변을 바탕으로 다음 질문을 이어나가고, 이야기가 충분히 구체화되었다고 판단되면 **최종적으로 다음과 같은 평가와 수정 방안을 구체적이고 세세하게, 그리고 쉽게 설명해줘.**\n"
